<footer>
  <div id="footer-container" class="container-fluid py-5">
    <div class="row footer mv-5">
      <!-- div class="col-sm-10 footer-menu-container text-center mx-auto">
              <h5>Testimonials</h5>
              <h5>About</h5>
              <h5>Contact</h5>
        </div -->
      </div>
    </div>
  <div class="footer-socmed-container bg-prm container-fluid">
    <div class="footer-socmed row mv-3 d-flex justify-content-between">
      <div class="col-3" style="min-width:fit-content">
        <p style="margin-top:0.5em;margin-bottom:0.5em; text-align: center">&copy; Alcoholics Recovered</p>
      </div>
      <!-- <div class="col-lg-6 footer-spacer"></div> -->
      <!-- <div class="col-auto float-right" style="max-width:200px">
        <img class="footer-socmed-icon" src="${pageContext.request.contextPath}/img/fb-white.png" alt="F"/>
        <img class="footer-socmed-icon" src="${pageContext.request.contextPath}/img/in-white.png" alt="I"/>
        <img class="footer-socmed-icon" src="${pageContext.request.contextPath}/img/tw-white.png" alt="T"/>
      </div> -->
    </div>
  </div>
 </footer>